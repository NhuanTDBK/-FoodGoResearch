...
from datasets import label_map_util


flags = tf.app.flags
flags.DEFINE_string('config_file','','Configuration file')
FLAGS = flags.FLAGS
...
def main(_):
  ...
  DATA_FOLDER = CONFIG["DATA_FOLDER"]
  N_LABELS = CONFIG["N_LABELS"]
  TFRECORD_FOLDER = CONFIG["TFRECORD_FOLDER"]
  LABEL_MAP_PATH = CONFIG["LABEL_MAP_PATH"]
  DATA_DIR = os.getcwd()
  ...
  writer = tf.python_io.TFRecordWriter(os.path.join(os.getcwd(),TFRECORD_FOLDER,"writer"))
  label_map_dict = label_map_util.get_label_map_dict(LABEL_MAP_PATH)
  logging.info('Reading from UEC dataset.')
  annotations_dir = os.path.join(ANNOTATION_FOLDER)
  examples_list = range(1,N_LABELS)
